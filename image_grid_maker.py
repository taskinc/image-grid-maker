...
HELP = {
    "folders": ("Add the folders that hold your photos. Tick 'Include sub-folders' "
                "to also scan nested folders. You can drag folders straight from "
                "your file explorer onto this window."),
    ...
    "selection": ("How many photos to place, and how to pick them. 'First N' takes "
                  "them in sorted order (sub-folder, then date, then name). "
...
class App(_BaseTk):
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(b, text="Include sub-folders",
                        variable=self.subfolders_var).pack(fill="x", pady=2)
        ...
        ttk.Label(gr, text="Number of photos to use:").grid(row=0, column=0, sticky="w", pady=4)
        self.count_var = tk.StringVar(value="100")
        e = ttk.Entry(gr, textvariable=self.count_var, width=10)
        e.grid(row=0, column=1, sticky="w", padx=6)
        e.bind("<KeyRelease>", lambda _e: self.preview_layout())
        self.avail_label = ttk.Label(gr, text="")
        self.avail_label.grid(row=0, column=2, sticky="w", padx=6)
        ttk.Label(gr, text="Method:").grid(row=1, column=0, sticky="w", pady=4)
        self.method_var = tk.StringVar(value="first")
        ttk.Radiobutton(gr, text="First N", variable=self.method_var,
                        value="first").grid(row=1, column=1, sticky="w", padx=6)
        ttk.Radiobutton(gr, text="Evenly spaced", variable=self.method_var,
                        value="evenly").grid(row=1, column=2, sticky="w", padx=6)
        ...
        ttk.Button(gr, text="Preview layout", command=self.preview_layout).grid(row=0, column=2, padx=6)
        self.layout_label = ttk.Label(f5, text="", wraplength=760, justify="left")
        self.layout_label.pack(anchor="w", padx=8, pady=2)
    ...
    def _add_folder_path(self, d):
        if d and os.path.isdir(d) and d not in self.folders:
    ...
    def _scan_worker(self):
        try:
            ...
            photos = core.scan_folders(self.folders, self.subfolders_var.get(),
                                       prog, workers=self.workers())
            if not photos:
    ...
    def refresh_selection(self):
        ...
        matching.sort(key=lambda p: p.sort_key())
    ...
    def _compute_layout(self):
        ...
        try:
            ...
            return None
        cols, rows, used = core.best_grid(count, len(self.matching), cell_ar, target_ar)
        return cols, rows, used, target_ar, cell_ar

    def preview_layout(self):
        res = self._compute_layout()
        if not res:
            return
        cols, rows, used, target_ar, cell_ar = res
        ...
        self.layout_label.config(
            text=("Grid %dx%d = %d photos  |  cell %dx%d px  |  output %dx%d px  "
                  "|  aspect %.3f (target %.3f)" %
                  (cols, rows, used, cw, ch, total_w, total_h, out_ar, target_ar)))
    ...
    def start_generate(self):
        ...
        threading.Thread(target=self._generate_worker,
                         args=(cols, rows, used, cell_ar, out_path), daemon=True).start()

    def _generate_worker(self, cols, rows, used, cell_ar, out_path):
        try:
            bw = max(0, self._int(self.border_var, 0))
            grid_w = max(cols + (cols + 1) * bw, self._int(self.width_var, 6000))
            cw, ch = core.cell_size_from_width(grid_w, cols, cell_ar, bw)

            self.log("Selecting %d photos (%s)..." %
                     (used, "evenly spaced" if self.method_var.get() == "evenly" else "first N"))
            selected = core.select_photos(self.matching, used, self.method_var.get())
            self.log("Compositing %dx%d grid, cell %dx%d px, %d workers..."
                     % (cols, rows, cw, ch, self.workers()))

            def prog(i, total):
                self.post(lambda: self.progress.config(maximum=total, value=i))

            img = core.build_grid(selected, cols, rows, cw, ch, cell_ar,
            ...
            self.log("Done: %dx%d px  ->  %s" % (w_px, h_px, out_path))
                    ...
                    (cols, rows, used, w_px, h_px, out_path))
